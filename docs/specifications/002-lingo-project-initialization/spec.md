...
global Integration lifecycle. Optional concepts may be absent or unconfigured;
...
| Repositories | At least one association with a stable Project-scoped key | Remote locator optional for local-only Repository; local paths never portable |
...
Only Project ID, Project name, at least one Repository association and
...
- **J1 — Minimal creation:** author chooses an empty target, enters name and one
  Repository association, omits all optional concepts or leaves them
...
- **FR-010 Guided flow:** generate identity once for a new Project and collect name,
  Repository associations, optional
...
1. **Portable validity:** supported schemaVersion, Project ID/name, at least one
...
|---|---|---|
| AC-01 | J1 creates and reopens a valid Project using only generated UUID v4, name, >=1 Repository association and schemaVersion; each required field enforced; all optional concepts may be absent/unconfigured | Black-box minimal round trip and unit required/optional-field matrix (FR-001, FR-010, FR-015) |
...
| AC-04 | Same init/install is no-op; rename/edit, Runtime change and copy preserve immutable UUID; different intent/ID, duplicate keys/exact or safely normalized remotes/checkouts, and ambiguous targets conflict | Unit UUID/duplicate cases and black-box conflict/relocation matrix; no independent fork identity (FR-001, FR-004, FR-012) |
